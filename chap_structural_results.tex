...
polynomials $A$ and $B$. 
Let us denote the quotient $A$ by $[u:v]$.
In a circuit, however, things get complicated due to multiple
...
\end{lemma}
\begin{proof}
The proof is by induction on the depth of $u$.\\

\noindent
1. 
The base case occurs when $\deg(u) \geq m$ but all its children have degree less than $m$, that is $u \in \mathcal{F}_m$. 
...
In fact, in the recent past, we have come pretty close to the required threshold and we shall see these lower bounds in the later chapters. \\
...
Note that in each iteration of the above procedure, we increase the top fan-in by a multiplicative factor of $s'$, and
what we gain is that some of the terms in the RHS will now have smaller degrees. 
...
Note that since we will always maintain homogeneity, the number of terms of degree at least $t/8$ in any summand is at most $8d/t$. 
...
We will be using this fact quite a lot so we state this explicitly as a corollary. 
...
\end{theorem}
We shall postpone  this theorem to a later time, in the interest of presenting more insight and intuition.
...
We will need the following lemma that was present in the survey of Shpilka and Yehudayoff \cite{sy} and also in the result of Hrube\v{s} and Yehudayoff \cite{HY11a} which we shall see a proof of in \autoref{chap:multilinear} as \autoref{lem:mult-logproduct} and \autoref{lem:hom-logproduct}.
...
\]
\end{theorem}
What this means is that even though the degree of all polynomials computed at depth 3 of the $\SPSP$ circuit have degree
bounded by $t$, each summand is a product of much more than $d/t$ factors. Another way to view this is that
\autoref{thm:av} gives a $\SPSP$ circuit of size $s^{O(d/t)}$ where the maximum bottom degree and the \emph{average bottom
    degree} are both bounded by $O(t)$. Whereas in the above theorem the maximum bottom degree is bounded by $t$ but the
\emph{average bottom degree} is bounded by $O(t / \log t)$. 
